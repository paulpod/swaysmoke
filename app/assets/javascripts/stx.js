...
(function(){
	...
	function _stx_js(_exports, _stxThirdParty) {
		...
		*/
		...
		STX.Market = function(market_definition) {
			...
			if ( typeof market_definition != 'undefined' && market_definition && market_definition != {}) {
				...
				/*if (market_definition.tz_lib) {
					this.tz_lib = market_definition.tz_lib;
				} else {
					this.tz_lib = timezoneJS.Date;					
				}*/
         ...
         * @param [Date=now] date An arbitrary date.
         * @return {Object} A date time for the open of a session or null if the market is
         * closed for the given date
         * @since  05-2016-10
         */
        STX.Market.prototype.getClose = function(date) {
            var d = new Date(date);
         ...
         * @since  05-2016-10
         */
        STX.Market.prototype.getNextClose = function(date) {
            var d = new Date(date);
            if (! this._wasOpenIntraDay(d)) {
         ...
         * @param [Date=now] date An arbitrary date.
         * @return {Date} A date aligned to the open time of the next open session.
         * @since  05-2016-10
         */
        STX.Market.prototype.getNextOpen = function(date) {
            var hours;
            var d = new Date(date);
            while (this._wasOpenIntraDay(d)) {
                hours = d.getHours() + 1;
                d.setHours(hours);
                if (hours == 24) { // maybe open 24 hours
                    d.setMinutes(59);
                    break;
                }
            }
        ...
        /**
         * Get the open date/time for a market session.
         * @param [Date=now] date An arbitrary date.
         * @return {Object} A date time for the open of a session or null if the market is
         * closed for the given date
         * @since  05-2016-10
         */
        STX.Market.prototype.getOpen = function(date) {
            var d = new Date(date);
            if (this._wasOpenIntraDay(d)) {
         ...
         * @param [Date=now] date An arbitrary date.
         * @param {Date} A date aligned to the previous close date/time of a session.
         * @since  05-2016-10
         */
        STX.Market.prototype.getPreviousClose = function(date) {
            var hours;
            var d = new Date(date);
            while (this._wasOpenIntraDay(d)) {
                hours = d.getHours() - 1;
                d.setHours(hours);
                if (hours == 0) { // maybe open 24 hours
                    d.setMinutes(1);
                    break;
                }
            }
            if (d.getHours() == this.zseg_match.close_parts.hours) {
                if (d.getMinutes() == this.zseg_match.close_parts.minutes) {
                    d.setHours(this.zseg_match.open_parts.hours);
                    d.setMinutes(this.zseg_match.open_parts.minutes);
                }
            }
         ...
         * @param {Date} date An arbitrary date.
         * @return {Date} A date aligned to previous open date/time of a session.
         * @since  05-2016-10
         */
        STX.Market.prototype.getPreviousOpen = function(date) {
            var d = new Date(date);
            var iter = this.newIterator({
                'begin': d,
                'interval': 1
            ...
            if (d.getHours() == this.zseg_match.open_parts.hours) {
                if (d.getMinutes() == this.zseg_match.open_parts.minutes) {
                    d = iter.previous();
                }
            }
            d.setHours(this.zseg_match.open_parts.hours);
		 ...
		 * @param {String} [parms.inZone] A valid timezone from the stxTimeZoneData.js library. This should represent the time zone for any input dates such as `parms.begin` in this function or `parms.end` in {@link STX.Market.Iterator#futureTick}. Defaults to browser timezone if none set.
		 * @param {String} [parms.outZone] A valid timezone from the stxTimeZoneData.js library. This should represent the time zone for the returned dates. Defaults to browser timezone if none set.
			...
			for (var i = 0; i < market.rules.length; i++) {
				...
				try {
					if (typeof rule.dayofweek !== 'undefined') {
						rule.year = "*";
						_TimeSegmentS._createDayOfWeekSegment(
							market, rule);
					} else if (typeof rule.date !== 'undefined') {
						rule.isDayOfWeek = false;
						rule.dayofweek = -1;
						_TimeSegmentS._createDateTimeSegement(market, rule);
					} else {
		...
		_TimeSegmentS._createDayOfWeekSegment = function(market, rule) {
			var data = {
				...
				'open': _TimeSegmentS._secSinceMidnight(market, rule.open),
				'close': _TimeSegmentS._secSinceMidnight(market, rule.close)
			};
			data.hash_code = this._hashCode(data.open + data.close);
		 ...
		 */
		_TimeSegmentS._secSinceMidnight = function(market, time_str) {
		 ...
		 * @param {Integer} skip Optional. Default is one. Move this many multiples of interval.
		 ...
		 * @param {Integer} skip Optional. The number of intervals to move. Defaults
		 * to one.
				...
				if(hasInput){
					...
					if(sd.libraryEntry && typeof sd.libraryEntry.edit!="undefined"){
						...
						editFunction=(function(stx, sd, inputs, outputs, parameters){return function(){
							stx.dispatch("studyOverlayEdit", {stx:stx,sd:sd,inputs:inputs,outputs:outputs, parameters:parameters});
						};})(stx, sd, inputs, outputs, parameters);
						sd.editFunction=editFunction;
					}else{
		...
		*/
		...
		STXChart.version=["Version 2016-05-10.1"];
		...
		function STXChart(config){
			...
			this.layout={
				...
				chartType: "candle",
			    /**
			     * Flag to enable extended hours timeframes
			     * @type boolean
			     * @default
			...
			this.preferences={
				...
				*/
				...
				magnet: false,
					 ...
					 *
					 * Note: for studies create a horizontalCrosshairFieldFN function that will
					 * be called by STX.Studies.addStudy. The function must return the field name
					 * in the dataSet to reference. The function will not be called when the study
					 * is set to overly or underlay the chart's panel.
					 *
					 * @example
		...
		STXChart.prototype.callbacks={
			...
			layout: null,
			/**
			 * Called when a drawing is added or deleted
			 * @type Function
		...
		STX.QuoteFeed.Demo.prototype.generateIntraday=function(params, cb){
			if(params.stx.marketFactory) {
				this.market = new STX.Market(params.stx.marketFactory(params));				
			} else {
