...
(function(){
	...
	function _stxLibrary_js(_stxThirdParty,_exports) {
		...
		STX.Studies.displayADX=function(stx, sd, quotes){
			STX.Studies.createYAxis(stx, sd, quotes, stx.panels[sd.panel]);
			if(sd.inputs.Series && sd.inputs.Shading){
				var parameters={
					topBand: "+DI " + sd.name,
					bottomBand: "-DI " + sd.name
				};
				STX.Studies.prepareIntersectingFill(stx, sd, quotes, parameters);
			}
			var opacity=sd.inputs.Series?0.4:1;
			if(sd.inputs.Histogram) STX.Studies.createHistogram(stx, sd, quotes, false, opacity);
			if(sd.inputs.Series!==false) STX.Studies.displaySeriesAsLine(stx, sd, quotes);
...

	{
		if ( typeof define === "function" && define.amd ) {
			...
			_stxLibrary_js(_stxThirdParty,_stxKernel);
		}
	}
